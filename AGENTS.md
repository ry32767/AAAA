...
- Project name: Maze Solver (MazeCreator)
- Purpose: 多階層の迷路を自動生成し、Paintbrush / 右手法 / 左手法 / BFS / DFS / A* など複数の解法アルゴリズムが解く様子を可視化する
- Target users: アルゴリズム学習者・教育用途、迷路や探索アルゴリズムに興味のある一般ユーザー
- Main value: 各探索アルゴリズムの挙動（探索順・経路・計算量）を視覚的・対話的に比較できること
- Public URL: https://ry32767.github.io/MazeCreator/
- Repository: https://github.com/ry32767/MazeCreator
- Repository type: `project pages`
- Production hosting: GitHub Pages（`main` への push で自動デプロイ）

### 技術スタック / 開発コマンド

- フロントエンド: 素の HTML / CSS / JavaScript（フレームワーク・ビルド不要、Canvas 描画）
- レガシー: `maze_game.py`（Tkinter 版、参考用に保持）
- テスト: Node.js 標準テストランナー（追加依存なし）。`npm test` で実行
- ビルド成果物: なし（リポジトリのファイルをそのまま Pages で配信）
- デプロイ: `.github/workflows/pages.yml`、CI テスト: `.github/workflows/ci.yml`

`app.js` はブラウザと Node の両方で読み込めるように作られています（ブラウザでは UI を初期化し、Node では純粋なアルゴリズム関数を `module.exports` で公開）。DOM に依存しないロジックを変更した場合は `npm test` で検証してください。
